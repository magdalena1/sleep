...
def get_rms_multiple_channels(book, selected_channels, channels_names, freq_spindle_range, window_width, percentile):
	...
	d, c = ss.butter(2, np.array([11, 16]) / (book.fs / 2), btype='bandpass')
	sig = ss.filtfilt(d, c, x)
	rms = []
	window = int(window_width * book.fs)
	for i in xrange(0, len(sig) - window, int(window)):
...
def create_profiles(f_name, data_dir):
	channels_names = channels_names_ears_smp
	#data_dir = '/home/mzieleniewska/sleep2/books/full_cap/'

	#f_name = 'MW_08_02_2017_ears_full_128_smp'
	# tags_file = '/home/mzieleniewska/sleep2/visual_marks/sleep_WS_21_04_2016.tag'

	patient = f_name #os.path.basename(f_name).split('.')[0]
	...
	b = BookImporter(os.path.join(data_dir, f_name + '.b'))
	atoms = b.atoms
	signals = b.signals
	epoch_len = b.epoch_s
	fs = b.fs
	...
	selected_channels = ['C3','Cz','C4','Pz','F3','Fz','F4']
	...
	#### mark sleep spindles
	channel_name, channel_id = select_channel(b, channels_names, selected_channels, freq_spindle_range)
	ids = patient_id + '_SS'
	try:
		with open(channels_db) as f:
			channels_data = json.load(f)
		if ids in channels_data.keys():
			selected_channel = channels_data[ids]
		else:
			channels_data[ids] = channel_name
			with open(channels_db, 'w') as f_out:
				json.dump(channels_data, f_out)
	...
	corrupted_epochs = mark_corrupted_epochs(b, channel_id)
	rms_ss, rms_ampli_ss = get_rms_multiple_channels(b, [channel_name], channels_names, freq_spindle_range, window_width, 97.0)

	chosen_SS = filter_atoms(os.path.join(out_dir, f_name + '_' + channel_name + '_SS.csv'), epoch_len, corrupted_epochs, ptspmV, atoms[channel_id], fs, freq_spindle_range, [0.4, np.inf], 1.0, [rms_ampli_ss, np.inf])
		
	recon = b._reconstruct_signal(channel_id, [0.5, 40])
	recon_spindle = b._reconstruct_signal(channel_id, freq_spindle_range)

	df = pd.read_csv(os.path.join(out_dir, f_name + '_' + channel_name + '_SS.csv'))
	offset = df['offset']
	width = df['width']
	amp = df['amplitude']
	ind_to_remove = []
	...
	df_new.to_csv(os.path.join(out_dir, f_name + '_' + channel_name + '_SS.csv'))

	#### mark rhythms
	chosen_delta = select_rhythm(b, channels_names, selected_channels, fs, freq_delta_range, window_width, 0.5, 97.0, out_dir, f_name, 'delta')
	chosen_theta = select_rhythm(b, channels_names, selected_channels, fs, freq_theta_range, window_width, 0.5, 97.0, out_dir, f_name, 'theta')
	chosen_alpha = select_rhythm(b, channels_names, selected_channels, fs, freq_alpha_range, window_width, 1.5, 97.0, out_dir, f_name, 'alpha')
	chosen_beta = select_rhythm(b, channels_names, selected_channels, fs, freq_beta_range, window_width, 0.4, 97.0, out_dir, f_name, 'beta')
	...
	# 	rms_theta, rms_ampli_theta = get_rms_multiple_channels(b, selected_channels, channels_names, freq_theta_range, window_width, 97.0)
	# 	rms_alpha, rms_ampli_alpha = get_rms_multiple_channels(b, selected_channels, channels_names, freq_alpha_range, window_width, 97.0)
	# 	rms_beta, rms_ampli_beta = get_rms_multiple_channels(b, selected_channels, channels_names, freq_beta_range, window_width, 97.0)
		
	# 	chosen_theta = filter_atoms(os.path.join(out_dir, f_name + '_' + ch + '_theta.csv'), epoch_len, corrupted_epochs, ptspmV, chann_atoms, fs, freq_theta_range, [0.5, np.inf], 1.0, [rms_ampli_theta, np.inf])
	# 	chosen_alpha = filter_atoms(os.path.join(out_dir, f_name + '_' + ch + '_alpha.csv'), epoch_len, corrupted_epochs, ptspmV, chann_atoms, fs, freq_alpha_range, [1.4, np.inf], 1.0, [rms_ampli_alpha, np.inf])
	# 	chosen_beta = filter_atoms(os.path.join(out_dir, f_name + '_' + ch + '_beta.csv'), epoch_len, corrupted_epochs, ptspmV, chann_atoms, fs, freq_beta_range, [0.4, np.inf], 1.0, [rms_ampli_beta, np.inf])

	# plot_pages_with_selected_atoms(b, f_name+'_' + ch + '_SS.csv', 'Cz', [11, 16], page_id=0)

	# signal = get_whole_signal(signals,selected_channels, epoch_len, number_of_epochs)

	# #### mark SWA
	# for i_ch, ch in enumerate(selected_channels):
	# 	idx = np.where(np.array(channels_names)==ch)
	# 	channel = idx[0][0]+1
	# 	energy, diff, corre, corrd = mark_corrupted_epochs(b, channel)
	# 	to_remove = set(corre).difference(corrd)
	...
	# 	rms_ampli_swa = 7*np.sqrt(np.median(signal[i_ch,:]**2)) 
	# 	print 'SWA amplitude 7 * median: ', rms_ampli_swa

	# 	chosen_SWA = filter_atoms(os.path.join(out_dir, f_name + '_' + ch + '_SWA.csv'), epoch_len, corrupted_epochs, ptspmV, chann_atoms, fs, [0.2,2], [0.5, 6], 1.0, [rms_ampli_swa, np.inf])

	# 	df = pd.read_csv(os.path.join(out_dir, f_name + '_' + ch + '_SWA.csv'))
	# 	offset = df['offset']
	# 	width = df['width']
	# 	amp = df['amplitude']
	# 	ind_to_remove = []
	...
	# 			ind_to_remove.append(i)
	# 	df_new = df.drop(df.index[ind_to_remove])
	# 	df_new.to_csv(os.path.join(out_dir, f_name + '_' + ch + '_SWA.csv'))

	# ## save selected channel for each patient for each structure based on mean energy of reconstructed structure
...
if __name__ == '__main__':
	...
	create_profiles(f_name, data_dir)
