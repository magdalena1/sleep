...
class TagsFileReader(object):
    ...
    def start_tags_reading(self):
        ...
        try:
            ...
            LOGGER.error("Couldn`t open tags file. {}".format(self._tags_file_name))
            raise IOError("Couldn`t open tags file. {}".format(self._tags_file_name))
        else:
            try:
            #Analyse xml info file, get what we want and close the file.
                ...
                raise IOError("An error occured while parsing tags xml file.")
            finally:
                l_tags_file.close()
