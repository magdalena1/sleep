...
def main(data_dir, f_name, channels_names, extension='mmp2'):
	artifact_file = os.path.join(data_dir, 'data', f_name+'_artifacts.tag')

	# channels_names = channels_names_ears_mmp
	...
	for ch_id in atoms.keys():
		...
		rms_ss, rms_ampli_ss = get_rms_multiple_channels(b, [channel_name], channels_names, freq_spindle_range, window_width, 97.0, apply_filter=True)
		print channel_name, ' ', rms_ampli_ss
		df[ch_id] = filter_atoms(os.path.join(out_dir, f_name + '_' + channel_name + '_SS.csv'), epoch_len, [], ptspmV, atoms[ch_id], fs, freq_spindle_range, [0.4, np.inf], 1.0, [rms_ampli_ss, np.inf])

	artifact_types = [u'muscle']
	df_clean = remove_structures(df, fs, epoch_len, number_of_epochs, artifact_file, [u'muscle'], 20.)
	svarog_tags_writer(df_clean, f_name+'_spindles_cleaned', os.path.join(data_dir, 'data'), fs, channels_names_ears_mmp, channels_names_ears_mmp)
...
if __name__ == '__main__':
	...
	# data_dir = '/home/mzieleniewska/empi/from_hpc/data/mmp2/patients/'
	data_dir = '/home/mzieleniewska/sleep2/books/one_channel/control'

	file_list = glob.glob(os.path.join(data_dir, 'data', "*.bin"))

	# for f in file_list:
	# 	f_name = os.path.split(f)[1].split('.')[0]
	main(data_dir, 'inb01', extension='smp')
